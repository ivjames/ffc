...
screen(id="pick", name="Course Picker", route="/golf/new",
 ...
 body="".join([
   ...
   box("empty state — “No courses at this location yet.” (replaces the list)","wide small cond",n=5),
...
screen(id="setup", name="Player Setup", route="/golf/new/setup",
 ...
 body="".join([
   ...
   row(icon("2"), box("tag well — error","inp err",n=3), txt("inline error — right of the input","muted"), cls="tagrow"),
   ...
   box("form error line — host/start failures (bare paragraph)","wide small cond"),
...
screen(id="play", name="Scorecard (play screen)", route="/golf/play/:clientId",
 ...
 specs=[
  ...
  (3,"Hole tablist","Row of hole tabs inside the scroll sheet (replaces the old “Holes” toggle)","one tab per hole, 36 tall × 48 wide; current bold + tinted","current / done / unplayed states; swipe or tap to move","--accent marks the current hole","the three tab states"),
  (4,"Player tag rail","Pinned tag chip per player (left rail)","radius 8 pill; 48-tall rows to track the grid","one row per player","--tag-accent","the tag surface (contrast-checked on any accent)"),
  (5,"Score cell","Score read-out per player per hole","48×48 cells in a ruled grid","punches on each stroke edit; ghost dot when unscored; active cell in the tinted column","--score column tint","the cell states"),
  (6,"± stepper rails","Pinned add / remove keys either side of the sheet","36×36, radius 8, in 48-tall rows","− disabled at 1, + disabled at the stroke cap","—","the key surface + the + / − marks"),
...
screen(id="tv", name="Leaderboard", route="/golf/leaderboard · /tv",
 ...
 body="".join([
   ...
   box("error banner — “needs the backend API” (above the announcement)","wide small cond",n=7),
 ...
 specs=[
  ...
  (7,"Board states","API-down error (amber, above the announcement) · empty · loading","text / amber banner","team vs player empty copy; error only when no data ever loaded; confetti on first populated load","(error emphasis)","—"),
 ]),
...
screen(id="putt", name="Arcade Putt", route="/arcade/putt", fill=True,
 purpose="Playable canvas mini-golf. A course picker precedes play; the playfield then fills the screen between the HUD and buttons. Offline.",
 body="".join([
   topbar("Arcade Putt", right=MENU()),
   box("COURSE PICKER (entry) — “Choose your course” · 4 course card rows (🏁 label / blurb / “9 holes · par n”) · ♾️ Endless (ghost)","wide tall","before play",n=1),
   row(txt("Hole n / 9 · course","inline"), txt("Par · Strokes","inline r"), cls="hud", n=2),
   ...
   btn("Next hole → / See scorecard → (after sinking)","primary",n=4),
   btn("Reset hole · + End run in endless (before sinking)","ghost"),
 ]),
 specs=[
  (1,"Course picker","Entry screen before play","stacked card rows + ghost button + note","one card per fixed course (The Classic / Creekside / The Dunes / The Gauntlet): 🏁 + bold label + blurb + “9 holes · par n”; “♾️ Endless (procedural)” ghost below; endless is fairness-checked","—","the entry surface + card rows + two mode glyphs"),
  (2,"Status header","Hole / par / strokes","text row","course (“/ 9 · course name”) vs endless (“· Endless”)","—","—"),
  ...
  (4,"Play buttons","A stacked column, phase-exclusive: after sinking only “Next hole → / See scorecard →”; before, “Reset hole” (+ “End run”, endless only)","full-width, stacked","a round-complete screen (🏆 + a scrolling per-hole scorecard + Play again / New run / Change mode) follows the last hole","--accent","primary + ghost surfaces + the round-complete card"),
 ]),
...
screen(id="food", name="Food & Drink menu", route="/food",
 ...
 specs=[
  ...
  (5,"Item sheet","Bottom-sheet modal opened from an item row","fixed overlay sheet","name + ✕ close; one block per modifier group (required / up-to-N / optional badges, option rows with +price/✓); − qty + stepper (1–20); “Add to cart · $total” disabled until required groups are picked","--accent (selected options)","the sheet surface + option-row states"),
 ]),
...
screen(id="order", name="Order status", route="/food/order/:orderId",
 ...
 specs=[
  ...
  (2,"Ready card","Pickup code (glowing) when ready","full-width card, radius 16","only at status=ready; “I’ve picked it up” → “Confirming…”; a pickup error renders inside this card, below its button","--glow accent","the card surface"),
...
screen(id="account", name="Account (sign in / profile)", route="/me/account",
 ...
 body="".join([
   ...
   btn("Sign in","primary sm"),
   btn("Use a different email","ghost sm"),
   btn("Resend the code","ghost sm"),
   txt("STAGE 3 — SIGNED IN","eyebrow"),
   box("“Signed in as” · email · Name · Arcade tag · Save / My teams / Sign out","wide tall","card",n=5),
 ]),
 specs=[
  (1,"Notice banners","Bonus / rounds-claimed messages — two separate cards (accent vs neutral border)","full-width, radius 16","role=status; appear above every stage when set","--accent (bonus card border)","banner surfaces"),
  ...
  (4,"Code stage","6-digit code entry + a stacked column of three buttons (Sign in / Use a different email / Resend the code)","wide arcade input; full-width stacked buttons","“Checking…”; Sign in disabled until 6 digits; expires in 10 min","—","the code field"),
...
screen(id="teams", name="Teams", route="/me/teams",
 ...
 specs=[
  ...
  (3,"New-team field","Name a new team","full-width input","maxlength 40; empty state above the field when no teams; errors (incl. team-list load failures) render as a bare line directly under this input","(error emphasis)","the field surface"),
...
screen(id="rewards", name="Rewards card", route="/me/rewards",
 ...
 specs=[
  (1,"Link-card state","Card-number entry when unlinked","full-width card, radius 16","“Looking up…”; 404 → “No card found with that number.”; a link failure shows the error + a ghost “Unlink and try another card”","—","the card surface"),
...
screen(id="photos", name="Photo Booth", route="/photos",
 ...
 body="".join([
   ...
   box("gallery error banner — load / read / delete failures","wide small cond"),
   ...
   btn("✅ Save to camera roll / Save changes","primary sm"),
   btn("📤 Share","ghost sm"),
   btn("✕ Cancel / Discard","ghost sm"),
 ]),
 specs=[
